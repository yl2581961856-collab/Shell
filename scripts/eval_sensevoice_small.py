...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="SenseVoice-small pseudo-streaming evaluator.")
    ...
    parser.add_argument("--model", default=os.getenv("SENSEVOICE_MODEL", ""), help="SenseVoice-small model path or id.")
    parser.add_argument("--device", default=os.getenv("SENSEVOICE_DEVICE", "npu:0"), help="Device string.")
    parser.add_argument(
        "--play",
        action="store_true",
...
def main() -> None:
    args = _parse_args()
    if not args.model:
        raise SystemExit("Missing --model or SENSEVOICE_MODEL")
    ...
    session_id = args.session_id.strip() or f"sv-{uuid.uuid4().hex[:12]}"
    _emit(
        {
            "type": "ack",
